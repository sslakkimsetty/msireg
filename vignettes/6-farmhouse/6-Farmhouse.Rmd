---
title: "Coregistration of MSI and H&E images of the Farmhouse painting dataset"
...
output:
    ...
    toc: true 
vignette: >
  %\VignetteIndexEntry{Coregistration of MSI and H&E images of the Farmhouse painting}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---


#### **This dataset is notable because it is non-biological. The farmhouse painting is affixed onto a glass slide and imaged. This dataset is included for its very low spatial resolution and having non-elastic deformations. It is also notable that the pig fetus tissue was ionized with a desorption electrospray ionization (DESI).** 
...
In this vignette, we illustrate the coregistration between mass spectrometry image and H&E images of the farmhouse painting which is affixed onto a glass slide. 

The MSI is acquired with a DESI ionization source using a LTQ linear ion trap mass spectrometer (Thermo Fannigan). The mass spectra are acquired between 100 and 1000 m/z for a total of 10,800 features. 
...
mse_data <- "farmhouse.rdata" 
opt_filename <- "farmhouse.png"  
env_filename <- "6-farmhouse.RData"
tf_filename <- "6-farmhouse.hdf5" 
``` 
...
load(file.path(DATAPATH, CURRPATH, mse_data)) 
mse_peaks <- as(farmhouse, "MSImagingExperiment") 
...
The MSI data we imported is already preprocessed. The peak picked and filtered data has 187 features. So, no further processing is needed. 
...
Now we select a `mz` value from the top features `topf`. `mz =` 323.25 is a good ion image to draw MSI issue outline. 

```{r viz-ssc-select-mz, eval=SHARING, echo=ECHO}
image(mse_peaks, mz=t323.25) 
...
# mse_roi <- multiSelectROI(mse_peaks, mz=323.25) 
``` 
