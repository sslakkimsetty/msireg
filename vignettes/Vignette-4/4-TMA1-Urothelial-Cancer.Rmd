---
title: "Coregistration of MSI and H&E images of Urothelial cancer TMA dataset"
...
output:
    ...
    toc: true 
vignette: >
  %\VignetteIndexEntry{Coregistration of MSI and H&E images of Urothelial cancer TMA dataset}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---


#### **This dataset is notable for its visually notable global and local deformations between the mass spectrometry and H\&E images. The high degree of misalignment is caused because the tissue microarray cores can easily move about independently.** 
...
In this vignette, we illustrate the coregistration between mass spectrometry image and H&E images of 29 tissue microarray (TMA) cores of urothelial cancer tissues. The dataset is publicly available via [PRIDE](https://www.ebi.ac.uk/pride/archive/projects/PXD026459). 

The MSI tissue was acquired with 4800 MALDI-TOF/TOF Analyzer, using the 4000 Series Explorer for instrumentation calibration. The raster step size of the image was 150 μm, resulting in 500 laser shots per spectrum. The mass spectra were captured in positive ion reflection mode with a mass range between 800 and 2300 m/z. After MSI acquisition and matrix removal, hemalum staining was performed to acquire the H&E image and tissues scanned using Axio Scan Z1 at x20 magnification (ZEISS, G ̈ottingen, Germany).
...
CURRPATH <- "6-TMA-urothelial-cancer/"
...
mse_filename <- "TMA1.imzML"
opt_filename <- "HE_stained_TMA1.TIF"
env_filename <- "4-TMA1-urothelial-cancer.RData"
tf_filename <- "4-TMA1-urothelial-cancer.hdf5"
``` 
...
# # Read ROI mask of optical image from file
# opt_roi <- read.table(file.path(DATAPATH, CURRPATH, opt_roi_filename),
#                       header=FALSE) |>
#     as.matrix() |> t()
# dimnames(opt_roi) <- NULL 

load(file.path(DATAPATH, CURRPATH, "TMA1-opt-dilated-roi.RData")) 

# Apply ROI mask on optical image
opt <- apply(imageData(opt), 3, function(x) x*opt_roi, simplify=FALSE) |>
    unlist() |> array(dim=dim(opt)) |> Image(colormode=Color)

opt <- flip(opt) 
opt_roi <- flip(opt_roi) 

load(file.path(DATAPATH, CURRPATH, "TMA1-mse-roi.RData")) 
mse <- mse[, t(mse_roi)]
```
...
mse_peaks <- readMSIData(file.path(DATAPATH, CURRPATH,
    "processed-in-R/TMA1-urothelial.imzML")) 
...
writeMSIData(mse_peaks, file.path(DATAPATH, CURRPATH,
        "processed-in-R/11-MUSC-2-liver-MALDI-first.imzML")) 
...
MSI's tissue outline can be computed in two ways. Often, if the tissue is imaged (likely in MALDI) where only the tissue areas are ionized and imaged, then the function `constructROIFromMSImage()` will automatically generate a mask separating background from the tissue. 

However, if the MSI includes background as well (for example, DESI), then `msireg` provides an in-built utility to generate the tissue ouline for MSI tissue. We select the tissue outline using the function `multiSelectROI` which is built on `Cardinal`'s `selectROI` method. `multiSelectROI` can select multiple disjoint tissue regions in one go. The method `multiSelectROI` needs an `mz` value to display a raster on which the tissue outline is selected. This is the case here. 
...
Now we select a `mz` value from the top features `topf`. `mz =` 805.4647 is a good ion image to draw MSI issue outline. 

```{r viz-ssc-select-mz, eval=SHARING, echo=ECHO}
image(mse_peaks, mz=805.4647) 
message("Appropriate m/z value for tissue outline selection: ", 805.4647) 
```
...
# mse_roi <- multiSelectROI(mse_peaks, mz=805.4647) 
``` 
...
The `ncc` values before and after coregistration are `r round(ncc_prereg, 3)` and `r round(ncc_postreg)` respectively. 
