...
opt_filename <- "48192_HeMat.png" 
opt_roi_filename <- "opt-roi.txt"
...
# opt_roi <- read.table(file.path(DATAPATH, CURRPATH, opt_roi_filename), header=FALSE) |>
#     as.matrix() |> t()
# dimnames(opt_roi) <- NULL 

load(file.path(DATAPATH, CURRPATH, "17-tonsil-48192-opt-roi.RData")) 

# Apply ROI mask on optical image
opt <- apply(imageData(opt), 3, function(x) t(x)*t(opt_roi), simplify=FALSE) |>
    unlist() |> array(dim=c(dim(opt)[2], dim(opt)[1], dim(opt)[3])) |> Image(colormode=Color) 
opt <- flop(opt) 
opt_roi <- opt_roi |> t() |> flop() 
```
...
mse_peaks <- readMSIData(file.path(DATAPATH, CURRPATH,
    "processed-in-R/17-48192-tonsil-peaks.imzML"))
...
Now we select a `mz` value from the top features `topf`. `mz =` 825.2533 is a good ion image to draw MSI issue outline. 

```{r viz-ssc-select-mz, eval=SHARING, echo=ECHO}
image(mse_peaks, mz=topf[3]) 
...
# mse_roi <- multiSelectROI(mse_peaks, mz=825.2533) 
``` 
