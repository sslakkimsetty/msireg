---
title: "Co-registration of HR2MSI mouse bladder dataset"
...
output:
    ...
    toc: true 
vignette: >
  %\VignetteIndexEntry{Coregistration of HR2MSI mouse bladder dataset}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---


#### **This dataset is notable for its high mass resolution, visually identifiable local distortions between MS image and its optical (microscopic) counterpart.** 
...
In this vignette, we illustrate the co-registration between phospholipid imaging of a mouse urinary bladder tissue and H&E stained microscopic image. The dataset is publicly available via [PRIDE](https://www.ebi.ac.uk/pride/archive/projects/PXD001283) and [MassIVE](https://massive.ucsd.edu/ProteoSAFe/dataset.jsp?task=117f527a97e3401baec21ecf050c6f99). 

The tissue was imaged with LTQ Orbitrap mass spectrometer with an AP-SMALDI imaging source. The step size (pixel size) is about 10$\mu$m. The mass resolving power of the instrument is 30,000 at m/z value 400 in positive ion mode. The mass range of the instrument is between m/z 100 and 1000. 
...
mse_filename <- "HR2MSI mouse urinary bladder S096.imzML"
opt_filename <- "HR2MSI mouse urinary bladder S096 - optical image.tiff"  
env_filename <- "5-mouse-bladder.RData"
tf_filename <- "5-mouse-bladder.hdf5"
``` 
...
If we already have a preprocessed MS image, we can load it here. Otherwise, skip this step. 
...
MSI's tissue outline can be computed in multiple ways. Often, if the tissue is imaged (likely in MALDI) where only the tissue areas are ionized and imaged, then the function `constructROIFromMSImage()` will automatically generate a mask separating background from the tissue. This is the case here. (For other ways to compute tissue outline, refer to Vignette 4.)
