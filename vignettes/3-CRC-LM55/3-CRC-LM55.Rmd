---
title: "Coregistration of MSI and H&E images of Colorectal cancer tissue (LM55)" 
...
In this vignette, we illustrate the coregistration between mass spectrometry image and H&E images of colorectal cancer tissue. The original study applied MALDI imaging to study the interpatient heterogeinity between primary tumors and liver metastases. In this workflow, we focus on one such primary tumor tissue (LM55). The dataset (and others in the study) is publicly available via [PRIDE](https://www.ebi.ac.uk/pride/archive/projects/PXD039409). 
...
topf <- topFeatures(ssc, n=20, model=list(s=24))$mz 
```

[! TODO set mz value that is appropriate for this dataset.] 

Now we select a `mz` value from the top features `topf`. `mz =` 853.3541 is a good ion image to draw MSI issue outline. 

```{r viz-ssc-select-mz, eval=SHARING, echo=ECHO}
image(mse_peaks, mz=853.3541) 
